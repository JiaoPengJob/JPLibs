...
public class JPIdCardUtil {
     ...
     * @param idCardNum
     */
    public JPIdCardUtil(String idCardNum) {
    ...
    public int isCorrect() {
        if (this.isEmpty()) {
            this.error = JPIdCardUtil.IS_EMPTY;
            return this.error;
        }

        if (!this.isLenCorrect()) {
            this.error = JPIdCardUtil.LEN_ERROR;
            return this.error;
        }

        if (!this.isCharCorrect()) {
            this.error = JPIdCardUtil.CHAR_ERROR;
            return this.error;
        }

        if (!this.isDateCorrect()) {
            this.error = JPIdCardUtil.DATE_ERROR;
            return this.error;
        }

        if (this.is18()) {
            if (!this.getCheck().equals(this.getCheckBit())) {
                this.error = JPIdCardUtil.CHECK_BIT_ERROR;
                return this.error;
            }
        }
