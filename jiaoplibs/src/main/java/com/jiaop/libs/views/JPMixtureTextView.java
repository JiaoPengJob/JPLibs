 ...
 * 富文本布局
 */
public class JPMixtureTextView extends RelativeLayout {
    ...
    public JPMixtureTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        readAttrs(context, attrs);
        mTextSize = (int) JPUIUtil.sp2px(context, 14f);
        if (mText == null) {
            mText = "Blank...";
        }
    ...
    private void cacuLineHeight() {
        layout = new StaticLayout("富文本", mTextPaint, 0, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0f, false);
    ...
    public void setTextSize(int unit, int size) {
        switch (unit) {
            ...
            case TypedValue.COMPLEX_UNIT_DIP:
                mTextSize = JPUIUtil.dp2PxInt(getContext(), size);
                break;
            case TypedValue.COMPLEX_UNIT_SP:
                mTextSize = JPUIUtil.sp2pxInt(getContext(), size);
                break;
        }
        mTextPaint.setTextSize(mTextSize);
        requestLayout();
